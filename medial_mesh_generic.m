function [] = medial_mesh_generic(fnvtk,fnmedout,fnbndout,labelSTJ,labelLVO,labelIAS)

% convert labelSTJ, labelLVO, and labelIAS to numbers
labelSTJ = str2double(labelSTJ);
labelLVO = str2double(labelLVO);
labelIAS = str2double(labelIAS);

% apply rigid transform to orient the outflow tract along the vertical axis
[pts_rotxyz, T_init] = reorient_pts(fnvtk, labelSTJ, labelLVO, labelIAS);
...
for i = 1 : nrot
    ...
    x_indices = round(pts_slab_pos(:,1));

    % Clamp the indices to ensure they are within the valid range
    z_indices = max(min(z_indices, size(img, 1)), 1);
    x_indices = max(min(x_indices, size(img, 2)), 1);

    ind = sub2ind(size(img), z_indices, x_indices);
    img(ind) = 1;
...
function [pts_rotxyz,T] = reorient_pts(fnvtk, labelSTJ, labelLVO, labelIAS)
